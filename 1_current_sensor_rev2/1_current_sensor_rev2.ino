...
unsigned long nextSendInterval = 2000;  //hardcoded now to 2000ms
...
void loop() {
  ...
  //ping reciever if tool is on
    if (toolOn && (now - lastOnSend >= nextSendInterval)) {
      //resend ping
      myData.x = 1;
      sendData();
      lastOnSend = now;
      Serial.println("Re-sent ping");
    }
