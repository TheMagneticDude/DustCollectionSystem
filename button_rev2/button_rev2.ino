...
unsigned long nextSendDelay = 2000; // hardcoded 2000 ms delay
...
void loop() {
  ...
  // === ON send while toolState is ON ===
  if (toolState) {
    if (millis() - lastSendTime >= nextSendDelay) {
      myData.x = 1;  //ON signal
      sendWithRetry((uint8_t *)&myData, sizeof(myData));
      Serial.println("Repeated ON ping sent");
      lastSendTime = millis();
    }
  } else {
  ...
  if (offPending && millis() - offRequestTime >= gracePeriodMillis) {
    //check if state stayed off during grace period 
